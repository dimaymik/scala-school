  ...
  case class StuffBox(books: List[Book] = Nil,
   ...
   def sortJunk(stuff: List[Stuff]): StuffBox = sort(stuff, StuffBox())
    ...
    private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox) = item match {
      case item@Watches(_, pr) if pr > 1000 => stuffBox.copy(watches = item :: stuffBox.watches)
      case junk@_ => stuffBox.copy(junk = junk :: stuffBox.junk)
      case item@Boots(brand, _) if brand == "Converse" || brand == "Adidas" => stuffBox.copy(boots = item :: stuffBox.boots)
      case item@Book(_, interest) if interest == true => stuffBox.copy(books = item :: stuffBox.books)
    }
